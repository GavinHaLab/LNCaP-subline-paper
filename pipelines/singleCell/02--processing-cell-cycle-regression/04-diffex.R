...
for(cluster in clusters) {
  ...
  result = RunPresto(so,ident.1 = cells, assay = 'RNA', slot = 'data', test.use = 'wilcox')  # use much faster presto implementation 
...
# *** arb *** using avg_diff since avg_log2FC is not produced when running scale.data

# order and write
out = as.data.frame(out)
out$order = as.numeric(out$pct.1)*as.numeric(out$avg_log2FC)*abs(as.numeric(out$pct.1)-as.numeric(out$pct.2))
# out$order = as.numeric(out$pct.1)*as.numeric(out$avg_diff)*abs(as.numeric(out$pct.1)-as.numeric(out$pct.2))
out = out[order(out$sample,out$cluster,-out$order),]

idx = out$pct.1 > 0.5 & out$p_val_adj < 0.05 & out$avg_log2FC > 0
#idx = out$pct.1 > 0.5 & out$p_val_adj < 0.05 & out$avg_diff > 0
