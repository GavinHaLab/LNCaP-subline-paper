...
inFile2 = 'intermediate/09b.txt'
outFile1 = 'figs/11-boxplot-numReads.png'
outFile2 = 'figs/11-boxplot-numGenes.png'
outFile3 = 'figs/11-boxplot-mito.png'
...
data2a = select(data2,sample,libraryType=SCSN)
plotData1 = merge(plotData,data2a,by='sample')
stopifnot(nrow(plotData) == nrow(plotData1))
plotData = plotData1
...
avg = prettyNum(round(mean(plotData$percentMito),1),big.mark=',')
title = paste0("Percent Mitochondrial Reads, Avg=",avg) 
subtitle = paste0(numSamples,' samples, ', numCells, ' cells')
p1 = ggplot(plotData,aes(x=sample,y=percentMito,fill=libraryType)) +
  theme_light(base_size=18) +
  theme(axis.text = element_text(color = 'black')) +   
  theme(axis.text.x = element_text(angle=45,vjust=1,hjust=1,size=10)) +
  theme(plot.title = element_text(size=14,hjust=.5)) +
  theme(plot.subtitle = element_text(size=14,hjust=.5)) +     
  geom_boxplot() +
  labs(x = '', y = '% Mito Reads',title=title,subtitle=subtitle) 

ggsave(outFile3,units='in',height=5,width=11,dpi=300)
print(outFile3)
