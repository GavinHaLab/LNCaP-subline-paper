...
library(dplyr)
source("/path/to/TitanCNA/R/utils.R")
...
geneFile <- args[2] #gene file with coordinates
sampleFile <- args[3] #list of samples to analyze, standard is purity_ploity.txt created by make_samplelist.py
method <- args[4] #{'common','severity','complete'}; common is the functional setting and what we use
headerType <- args[5] #{'Gene','chrPosn'}
filterLen <- as.numeric(args[6]) #filter by length
outPrefix <- args[7] #output prefix for matrices
